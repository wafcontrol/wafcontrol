# Detect Attack When Apache Webserver in Enabled
...
def filter_rule_hits(hits: List[Tuple[str, str]]) -> List[Tuple[str, str]]:
    ...
    for rid, msg in hits:
        ...
        if family not in IMPORTANT_FAMILIES:
            continue
...
def discover_apache_audit_targets() -> Tuple[List[str], List[str]]:
    files = []; roots = ["/etc/apache2/apache2.conf", "/etc/httpd/conf/httpd.conf", "/etc/apache2/sites-enabled/*", "/etc/apache2/conf-available/*.conf", "/etc/httpd/conf.d/*.conf", "/etc/apache2/modsecurity.d/*.conf", "/etc/modsecurity/*.conf", "/etc/apache2/mods-enabled/*.conf"]
    confs = []
    for pat in roots:
...
def only_summary_families(hits: List[Tuple[str, str]]) -> bool:
    if not hits:
        return True
    for rid, _ in hits:
        fam = (rid or "")[:3]
        if fam and fam not in ("949", "920"):
            return False
    return True
